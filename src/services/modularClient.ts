...
export interface ModularClientOptions {
    ...
    heartbeatInterval?: number; // in milliseconds, default 30000
...
export class ModularClient extends EventEmitter {
    ...
    async register(): Promise<ModularApiResponse> {
        try {
    ...
    async unregister(): Promise<ModularApiResponse> {
        try {
            this.stopHeartbeat();
    ...
    async sendHeartbeat(customHealthData?: Partial<ModuleHealth>): Promise<ModularApiResponse> {
        if (!this.isRegistered) {
            throw new Error('Module not registered');
        }

        try {
            const healthData: Partial<ModuleHealth> = {
                ...customHealthData,
            ...
            const response = await this.makeRequest(
                'POST',
                `/heartbeat/${this.moduleConfig.moduleId}`, 
                healthData
            );
    ...
    async pollForCommunications(): Promise<void> {
    ...
    async sendDataToCMS(action: string, data: any): Promise<ModularApiResponse> {
        try {
    ...
    async getModuleInfo(): Promise<ModularApiResponse> {
        try {
    ...
    async getAllModules(): Promise<ModularApiResponse> {
        try {
    ...
    async getSystemHealth(): Promise<ModularApiResponse> {
        try {
    ...
    private async retryRequest(requestFn: Function): Promise<any> {
        let lastError: any;
...
export function createModularClient(options: ModularClientOptions): ModularClient {
    return new ModularClient(options);
}

// Example usage helper
export function createExampleModuleConfig(moduleId: string, moduleName: string): ModuleConfig {
    return {
        moduleId,
        moduleName,
        version: '1.0.0',
