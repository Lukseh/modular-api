...
import { ModuleConfig } from '../types/modular';
  ...
  switch (format) {
    case 'csv':
      ...
      const csvRows = processedData.map(item => 
        `${item.id},"${item.data}",${item.processedAt}`
      ).join('\n');
    ...
    case 'xml':
      const xmlData = processedData.map(item => 
        `<item><id>${item.id}</id><data>${item.data}</data><processedAt>${item.processedAt}</processedAt></item>`
      ).join('');
